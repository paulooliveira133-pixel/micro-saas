...
app.use("/api/saas/register", rateLimit({
...
app.use("/api", async (req: any, res, next) => {
  ...
  if (!tenantId || req.path.startsWith("/saas") || req.path.startsWith("/auth") || req.path.startsWith("/pagamento") || req.path.startsWith("/relatorio")) {
    return next();
  }
...
app.get("/api/auth/trial-status", async (req: any, res) => {
  ...
  if (tenant.planStatus === "active") return res.json({ status: "active", daysLeft: null, expired: false });
  if (now <= trialEndsAt) return res.json({ status: "trial", daysLeft: Math.max(0, daysLeft), expired: false });
...
app.get("/api/establishment", async (req: any, res) => {
  ...
  res.json({
    id: tenant.id, name: tenant.name, phone: tenant.phone,
    openTime: tenant.openTime, closeTime: tenant.closeTime,
    address: tenant.address, whatsappApiKey: tenant.whatsappApiKey,
    webhookUrl: tenant.webhookUrl, logoUrl: tenant.logoUrl,
    theme: tenant.theme, customDomain: tenant.customDomain,
...
app.get("/api/services", async (req: any, res) => {
  const services = await prisma.service.findMany({ where: { tenantId: req.tenant.id }, orderBy: { createdAt: "asc" } });
  res.json(services);
});
...
app.post("/api/services", async (req: any, res) => {
  ...
  const service = await prisma.service.create({
    data: { name: parsed.data.name, durationMin: parsed.data.durationMin || 30, price: parsed.data.price || 50, tenantId: req.tenant.id },
  });
  res.json(service);
});
...
app.get("/api/professionals", async (req: any, res) => {
  res.json(await prisma.professional.findMany({ where: { tenantId: req.tenant.id } }));
});
  ...
  if (tenant.webhookUrl && tenant.whatsappApiKey) {
    try {
      await fetch(tenant.webhookUrl, {
        ...
        body: JSON.stringify({ phone: parsed.data.customerPhone, message: `Olá ${parsed.data.customerName}! Seu agendamento está confirmado para ${parsed.data.date} às ${parsed.data.time}. Esperamos você!` }),
      });
    } catch (e) {}
  }
...
app.get("/api/notifications", async (req: any, res) => {
  const notifications = await prisma.notification.findMany({ where: { tenantId: req.tenant.id }, orderBy: { createdAt: "desc" } });
  res.json(notifications.map(n => ({ id: n.id, appointmentId: n.appointmentId, recipient: n.recipient, recipientPhone: n.recipientPhone, message: n.message, status: n.status, timestamp: n.createdAt, type: n.type })));
});

app.post("/api/notifications/simulate", async (req: any, res) => {
  const notification = await prisma.notification.create({
    data: { type: req.body.type || "reminder", recipient: "Cliente", recipientPhone: "+55 (11) 99999-0000", message: "Lembrete de agendamento simulado.", status: "enviado", appointmentId: req.body.appointmentId || "apt-sim", tenantId: req.tenant.id },
  });
...
app.get("/api/plans", async (req: any, res) => {
  const plans = await prisma.plan.findMany({ where: { tenantId: req.tenant.id }, include: { services: true } });
  res.json(plans.map(p => ({ ...p, servicesIncluded: p.services.map(s => s.id) })));
});

app.post("/api/plans", async (req: any, res) => {
  const plan = await prisma.plan.create({
    data: { name: req.body.name, price: req.body.price, description: req.body.description || "", limitCount: req.body.limitCount || 4, tenantId: req.tenant.id, services: { connect: (req.body.servicesIncluded || []).map((id: string) => ({ id })) } },
  });
  res.json(plan);
});
...
app.get("/api/subscribers", async (req: any, res) => {
  res.json(await prisma.subscriber.findMany({ where: { tenantId: req.tenant.id } }));
});
...
app.get("/api/saas/tenants", async (req, res) => {
  const tenants = await prisma.tenant.findMany({ include: { professionals: true, services: true, appointments: true, subscribers: true } });
  res.json(tenants.map(t => ({ id: t.slug, slug: t.slug, establishment: { name: t.name, phone: t.phone }, professionals: t.professionals, services: t.services, appointments: t.appointments, subscribers: t.subscribers })));
});
...
app.post("/api/saas/tenants", async (req, res) => {
  ...
  const tenant = await prisma.tenant.create({
    data: { name, slug, phone: phone || "", address: address || "", openTime: openTime || "09:00", closeTime: closeTime || "20:00", adminUsername: adminUsername || "gerente", adminPassword: adminPassword || "gerente123" },
  });
  ...
  try {
    ...
    const r = await fetch("https://api.mercadopago.com/checkout/preferences", {
      ...
      body: JSON.stringify({
        ...
        back_urls: { success: `${process.env.APP_URL || "https://autodireto.online"}/?payment=success`, failure: `${process.env.APP_URL || "https://autodireto.online"}/?payment=failure`, pending: `${process.env.APP_URL || "https://autodireto.online"}/?payment=pending` },
      }),
    });
...
app.post("/api/ai/analyze", async (req: any, res) => {
  const tenantId = req.headers["x-tenant-id"] as string;
  const tenant = await prisma.tenant.findUnique({ where: { slug: tenantId }, include: { services: true, appointments: true } });
  const claudeKey = process.env.ANTHROPIC_API_KEY;
  const summaryStr = JSON.stringify({ salonName: tenant?.name, services: tenant?.services.map(s => ({ name: s.name, price: s.price })), appointments: tenant?.appointments.map(a => ({ customer: a.customerName, date: a.date, status: a.status })) });
  ...
  if (claudeKey) {
    try {
      ...
      return res.json({ provider: "Claude Sonnet 4 (Anthropic)", data: JSON.parse(clean) });
    } catch (err) { console.error("Claude error:", err); }
  }
  return res.json({
    provider: "Smart Local Engine",
    data: {
      churnReport: { atRiskCount: 1, atRiskCustomers: [{ name: "Thiago Lima", phone: "+55 (11) 93333-4444", lastVisit: "2026-05-20", reason: "Agendamento cancelado sem nova reserva.", retentionAction: "Enviar cupom de 15% via WhatsApp." }], generalStatus: "Taxa de retenção saudável." },
      slotOptimization: { recommendedFillerDeals: ["Desconto de 15% nas terças pela manhã."], suggestedQuietHoursPromo: "Café cortesia nas terças.", forecastedBusyDays: ["Sexta-feira", "Sábado"] },
    },
  });
});
...
app.get("/api/relatorio/pdf", async (req: any, res) => {
  ...
  const tenant = await prisma.tenant.findUnique({
    where: { slug: tenantId },
    include: { appointments: { include: { service: true, professional: true }, orderBy: { createdAt: "desc" } }, services: true, subscribers: { include: { plan: true } } },
  });
  if (!tenant) return res.status(404).json({ error: "Tenant não encontrado" });
  ...
  const gold = "#C9A84C"; const dark = "#1A1A1A"; const gray = "#666666"; const light = "#999999";
  ...
  const cardY = doc.y; const cardW = (doc.page.width - 120) / 4;
  [{ label: "Total Agendamentos", value: String(apts.length), color: gold }, { label: "Concluídos", value: String(concluded.length), color: "#4CAF50" }, { label: "Cancelados", value: String(canceled.length), color: "#E57373" }, { label: "Faturamento", value: `R$ ${totalRevenue.toFixed(2).replace(".", ",")}`, color: gold }]
    .forEach((card, i) => {
  ...
  doc.moveDown(5.5);
  doc.fontSize(9).font("Helvetica").fillColor(gray).text(`Ticket médio: R$ ${avgTicket.toFixed(2).replace(".", ",")}   |   Taxa de cancelamento: ${apts.length > 0 ? ((canceled.length / apts.length) * 100).toFixed(1) : 0}%   |   Pendentes: ${pending.length}`);
  doc.moveDown(1.5);
  ...
  apts.forEach(a => { if (a.service) { if (!serviceCount[a.service.name]) serviceCount[a.service.name] = { count: 0, revenue: 0 }; serviceCount[a.service.name].count++; if (a.status === "concluido" || a.status === "concluído") serviceCount[a.service.name].revenue += a.service.price; } });
  ...
  if (sortedServices.length === 0) {
    ...
    doc.text("Serviço", colX[0], doc.y); doc.text("Agendamentos", colX[1], doc.y - doc.currentLineHeight()); doc.text("Receita", colX[2], doc.y - doc.currentLineHeight()); doc.text("% do total", colX[3], doc.y - doc.currentLineHeight());
    doc.moveDown(0.3); doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor("#DDDDDD").lineWidth(0.5).stroke(); doc.moveDown(0.4);
    sortedServices.slice(0, 8).forEach(([name, data], idx) => {
      ...
      doc.fontSize(9).font("Helvetica").fillColor(dark).text(name, colX[0], rowY, { width: 160 }); doc.text(String(data.count), colX[1], rowY); doc.text(`R$ ${data.revenue.toFixed(2).replace(".", ",")}`, colX[2], rowY); doc.text(`${pct}%`, colX[3], rowY); doc.moveDown(0.8);
    });
  }
  ...
  doc.moveDown(0.3); doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor(gold).lineWidth(1).stroke(); doc.moveDown(0.8);
  ...
  if (apts.length === 0) {
    ...
    doc.fontSize(8).font("Helvetica-Bold").fillColor(gray);
    doc.text("Data", cols[0], doc.y); doc.text("Hora", cols[1], doc.y - doc.currentLineHeight()); doc.text("Cliente", cols[2], doc.y - doc.currentLineHeight()); doc.text("Serviço", cols[3], doc.y - doc.currentLineHeight()); doc.text("Status", cols[4], doc.y - doc.currentLineHeight());
    doc.moveDown(0.3); doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor("#DDDDDD").lineWidth(0.5).stroke(); doc.moveDown(0.4);
    apts.slice(0, 15).forEach((apt, idx) => {
      if (doc.y > 720) doc.addPage();
      const rowY = doc.y;
      if (idx % 2 === 0) doc.rect(50, rowY - 3, doc.page.width - 100, 18).fill("#FAFAFA");
      const statusColor = apt.status === "concluido" || apt.status === "concluído" ? "#4CAF50" : apt.status === "cancelado" ? "#E57373" : gold;
      doc.fontSize(8).font("Helvetica").fillColor(dark).text(apt.date || "-", cols[0], rowY, { width: 75 }); doc.text(apt.time || "-", cols[1], rowY, { width: 80 }); doc.text(apt.customerName || "-", cols[2], rowY, { width: 105 }); doc.text(apt.service?.name || "-", cols[3], rowY, { width: 95 }); doc.fillColor(statusColor).text(apt.status || "-", cols[4], rowY, { width: 80 }); doc.moveDown(0.8);
    });
  }

  if (tenant.subscribers.length > 0) {
    if (doc.y > 600) doc.addPage();
    doc.moveDown(1); doc.fontSize(13).font("Helvetica-Bold").fillColor(dark).text("Assinantes Ativos");
    doc.moveDown(0.3); doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor(gold).lineWidth(1).stroke(); doc.moveDown(0.8);
    const subCols = [50, 220, 330, 430];
    doc.fontSize(8).font("Helvetica-Bold").fillColor(gray);
    doc.text("Cliente", subCols[0], doc.y); doc.text("Telefone", subCols[1], doc.y - doc.currentLineHeight()); doc.text("Plano", subCols[2], doc.y - doc.currentLineHeight()); doc.text("Status", subCols[3], doc.y - doc.currentLineHeight());
    doc.moveDown(0.3); doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor("#DDDDDD").lineWidth(0.5).stroke(); doc.moveDown(0.4);
    tenant.subscribers.slice(0, 20).forEach((sub, idx) => {
      if (doc.y > 720) doc.addPage();
      const rowY = doc.y;
      if (idx % 2 === 0) doc.rect(50, rowY - 3, doc.page.width - 100, 18).fill("#FAFAFA");
      doc.fontSize(8).font("Helvetica").fillColor(dark).text(sub.customerName, subCols[0], rowY, { width: 165 }); doc.text(sub.customerPhone, subCols[1], rowY, { width: 105 }); doc.text(sub.plan?.name || "-", subCols[2], rowY, { width: 95 }); doc.fillColor(sub.status === "ativo" ? "#4CAF50" : "#E57373").text(sub.status, subCols[3], rowY); doc.moveDown(0.8);
    });
  }
  ...
  for (let i = 0; i < pageCount; i++) {
    doc.switchToPage(i);
    doc.fontSize(8).font("Helvetica").fillColor(light).text(`AgendaFácil · ${tenant.name} · Página ${i + 1} de ${pageCount}`, 50, doc.page.height - 40, { align: "center", width: doc.page.width - 100 });
